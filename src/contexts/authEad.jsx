...
import {api, loginEadSession, getDisciplinas, getAllTheme, getEducator, updateStatus} from "../services/api";
...
export const AuthEadProvider = ({children}) =>{
    ...
    const logout = () => {
     ...
     useEffect(() => { 
        ...
        if (recoveredUser && token &&  JSON.parse(recoveredUser).permission === "aluno") {
    ...
    return (
        <AuthEadContext.Provider value={{ authenticated: !!user, user, loading, login, logout , disciplinas, themes , listEducator, updatetStatusTheme}}>
            {children}
